...
export const buildPlutoImageUrl = (
    ...
    if (width > 0) params.width = String(width)
    if (height > 0) params.height = String(height)
    params.type = type != '' ? type : 'webp'
    params.quality = (quality > 0 ? quality : 80).toString();

    const searchParams = new URLSearchParams(params)
    const queryString = lossless
