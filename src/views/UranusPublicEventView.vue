<template>
  ...
  <div class="uranus-public-page">
    ...
    <!-- Event content -->
    <div v-else-if="event" class="uranus-public-event-detail-layout">
...
import { UranusPublicEventDetail, UranusPublicEventDate } from '@/models/UranusPublicEventModel.ts'
...
const eventStartDate = computed(() => eventDate.value?.startDate ?? event.value?.startDate ?? null)
  ...
  try {
    ...
    eventDate.value = mappedEvent.furtherDates.find(fd => fd.dateId === Number(eventDateId))
        ?? new UranusPublicEventDate(
            ...
            null,
            mappedEvent.venueId,
            mappedEvent.venueName,
            mappedEvent.venueStreet,
            mappedEvent.venueHouseNumber,
            mappedEvent.venuePostalCode,
            mappedEvent.venueCity,
            mappedEvent.venueCountryCode,
            mappedEvent.venueStateCode,
            mappedEvent.venueLat,
            mappedEvent.venueLon,
            ...
            mappedEvent.locationHouseNumber,
            mappedEvent.locationPostalCode,
            mappedEvent.locationCity,
            mappedEvent.locationCountryCode,
            mappedEvent.locationStateCode,
            mappedEvent.locationLat,
...
const onDownloadIcs = async () => {
  if (!event.value?.eventId || !eventDate.value?.dateId) {
    return
  }
  ...
  try {
    ...
    const eventDateId = eventDate.value.dateId
