...
const buildPopupHtml = (properties: PopupProperties, coordinates: [number, number]): string => {
  ...
  const titleHtml = website
    ? `<a class="uranus-map-popup__title-link" href="${website}" target="_blank" rel="noopener noreferrer">${escapeHtml(
        title
      )}</a>`
  ...
  const metaHtml = metaLines
    .map(
      (line) => `
        <p class="uranus-map-popup__meta">
          ${escapeHtml(line)}
        </p>
      `
    )
  ...
  const detailsLink = website
    ? `<a class="uranus-map-popup__action uranus-map-popup__action--details" href="${website}" target="_blank" rel="noopener noreferrer">
        <span>Link</span>
      </a>`
    : ''
  ...
  return `
    <div class="uranus-map-popup">
      ...
      <div class="uranus-map-popup__body">
        ${metaHtml || '<p class="uranus-map-popup__meta">Keine weiteren Angaben</p>'}
      </div>
      <div class="uranus-map-popup__footer">
        <a class="uranus-map-popup__action uranus-map-popup__action--route" href="${directionsUrl}" target="_blank" rel="noopener noreferrer">
          <span>Route</span>
        </a>
        ${detailsLink}
      </div>
...
<style>
/* Global style for MapLibre popups (cannot be scoped) */
