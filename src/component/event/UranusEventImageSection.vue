...
const local = ref<Slice>({
  ...
  image_created: props.modelValue?.image_created ?? '',
  image_copyright: props.modelValue?.image_copyright ?? '',
  image_license: props.modelValue?.image_license ?? null,
...
async function save() {
  ...
  try {
    ...
    fd.append('alt_text', local.value.image_alt ?? '')
    fd.append('created_by', local.value.image_created ?? '')
    fd.append('copyright', local.value.image_copyright ?? '')
    if (local.value.image_license != null) fd.append('license', String(local.value.image_license))
