<!--
  src/view/UranusSpaceEditForm.vue
-->
<template>
              ...
              <div class="accessibility-flags">
                ...
                >
                  <UranusFlagsCheck
                      ...
                      :label="t(flag.name)"
                      :model-value="flagComputed(flag.id).value"
                      @update:model-value="flagComputed(flag.id).value = $event"
                  />
                </div>
              </div>
            </section>
...
import { useBigIntFlags } from '@/composable/useBigIntFlags.ts'
...
import UranusFlagsCheck from "@/component/ui/UranusFlagsCheck.vue";
...
const { value: accessibilityFlags, flagComputed } = useBigIntFlags(0n)
...
const applyInitialValues = (values?: SpaceFormInitialValues) => {
  ...
  if (values?.accessibilityFlags != null) {
    accessibilityFlags.value = BigInt(values.accessibilityFlags)
  } else {
    accessibilityFlags.value = 0n
  }
...
const handleSubmit = () => {
  ...
  const accessibilityMask = accessibilityFlags.value
    ...
    emit('submit', {
      ...
      accessibilityFlags: accessibilityMask !== 0n ? accessibilityMask.toString() : null,
    })
}
