<template>
  <div class="uranus-main-layout">
    <UranusDashboardHero
        :title="t('organization_team_management')"
        :subtitle="t('organization_team_management_description')"
    />

    <section class="organization-team">
      <UranusFeedback v-if="isLoading" :message="t('organization_team_loading')" type="warning" />
      <UranusFeedback v-else-if="loadError" :message="loadError" type="error" />
        ...
        <article class="uranus-card uranus-form team-card team-card--members">
          <header class="team-card__header">
            ...
            <button
                ...
                class="uranus-secondary-button team-reset-button"
          ...
          <ul v-if="filteredMembers.length" class="team-member-list">
            <li v-for="member in filteredMembers" :key="member.user_uuid" class="team-member">
        ...
        <form class="team-form uranus-form" @submit.prevent="submitInvite">
          ...
          <UranusFormRow>
            <UranusTextfield
                ...
                v-model="inviteEmail"
                type="email"
                :label="t('organization_team_invite_email_label')"
                autocomplete="email"
                required />
        ...
        <section class="team-pending">
          ...
          <ul v-if="pendingInvitations.length" class="team-pending-list">
            <li
                ...
                :key="invite!.invited_by!"
                class="team-pending-item">
              <div>
                ...
                <button
                    ...
                    type="button"
                    :disabled="inviteActionUuid === invite.user_uuid" @click="cancelInvite(invite)">
                    <span v-if="inviteActionUuid === invite.invited_by">{{ t('organization_team_removing') }}</span>
...
import type { UranusOrganizationTeamMemberDTO } from '@/model/uranusOrganization.ts'
...
const members = ref<UranusOrganizationTeamMemberDTO[]>([])
...
const memberActionUuid = ref<string | null>(null)
const inviteActionUuid = ref<string | null>(null)
...
const loadTeam = async () => {
  ...
  try {
    const { response } = await apiFetch<any>(
      `/api/admin/organization/${orgUuid.value}/team?lang=${locale.value}`
    )
...
const removeMember = async (member: UranusOrganizationTeamMemberDTO) => {
    ...
    memberActionUuid.value = member.user_uuid

    try {
        await apiFetch(`/api/admin/organization/${orgUuid.value}/team/member/${member.user_uuid}`, {
            method: 'DELETE',
        })

        members.value = members.value.filter((entry) => entry.user_uuid !== member.user_uuid)
    } catch (err) {
...
const submitInvite = async () => {
  console.log('submitInvite')
    ...
    try {
        await apiFetch(`/api/admin/organization/${orgUuid.value}/team/invite?lang=${locale.value}`, {
            method: 'POST',
...
const cancelInvite = async (invite: OrganizationTeamInvitation) => {
    ...
    inviteActionUuid.value = invite.user_uuid

    try {
        await apiFetch(`/api/admin/organization/${orgUuid.value}/team/member/${invite.user_uuid}`, {
                method: 'DELETE',
            }
        )

        invitations.value = invitations.value.filter(
            (entry) => entry.user_uuid !== invite.user_uuid
        )
    } catch (err) {
