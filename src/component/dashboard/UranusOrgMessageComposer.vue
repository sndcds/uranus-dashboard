<template>
  <section class="composer-panel">
    <div v-if="!selectedOrg" class="inbox-placeholder">
    ...
    >
      <header class="message-form__header">
        <div class="message-recipient">
          ...
          <strong class="message-recipient__name">{{ selectedOrg.name }}</strong>
          <span v-if="selectedOrg.email" class="message-recipient__email">
            {{ selectedOrg.email }}
          </span>
          <span v-if="selectedOrgLocation" class="message-recipient__meta">
            {{ selectedOrgLocation }}
          </span>
        </div>
      </header>
...
const props = defineProps<{
  ...
  selectedOrgLocation: string | null;
}>()
...
const sendMessage = async () => {
  ...
  try {
    const rawContextId =
      typeof props.selectedOrg.id === 'number'
        ? props.selectedOrg.id
        : Number.parseInt(String(props.selectedOrg.id), 10)
