    ...
    <div class="event-dates">
      <div v-for="(date, index) in localDates" :key="index" class="event-dates__card">
        ...
        <div class="event-dates__grid">
          <UranusDateInput :id="`start-date-${index}`" v-model="date.startDate" :label="t('event_start_date')"
            required />

          <UranusTimeInput :id="`start-time-${index}`" v-model="date.startTime" :label="t('event_start_time')"
            required />
          ...
          <label :for="`all-day-${index}`" class="checkbox-label">
            <input type="checkbox" :id="`all-day-${index}`" v-model="date.allDayEvent" class="checkbox-input" />
          ...
          <UranusFieldLabel :id="`venue-${index}`" :label="t('event_venue_label')">
            <select :id="`venue-${index}`" v-model="date.venueId" @change="() => onVenueSelected(date.venueId, date)">
...
.checkbox-label {
  ...
  /* inline so it doesn't force a new line */
...
.checkbox-text {
  ...
  vertical-align: middle !important;
  white-space: nowrap;
  /* optional: keeps text on same line */
}
