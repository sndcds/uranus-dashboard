...
const props = defineProps<{ modelValue?: UranusEventUrl[] }>()
const emit = defineEmits<{ (e: 'update:modelValue', v: UranusEventUrl[]): void; (e: 'updated'): void }>()
const local = ref<UranusEventUrl[]>(props.modelValue ? props.modelValue.map(x => ({ ...x })) : [])
